...
namespace VRStandardAssets.Maze
{
    ...
    */
    public class DestinationMarker : MonoBehaviour
    {
        ...
        // once the marker is set handle it
        private void OnEnable ()
        {
            m_MazeTargetSetting.OnTargetSet += HandleTargetSet;
        }

        // stop handling the maerker when the player gets to it
        private void OnDisable ()
        {
            m_MazeTargetSetting.OnTargetSet -= HandleTargetSet;
        }

        // when the marker is not being used dont render it 
        public void Hide()
        {
            m_Renderer.enabled = false;
        }

        // while the render is being used render the marker
        private void Show()
        {
